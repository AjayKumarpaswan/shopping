...
const Kurta = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await axios.get("http://localhost:5000/api/products/category/Kurta");
        setProducts(res.data);
      } catch (error) {
        console.error("Failed to fetch Kurta products:", error);
      }
    };
    fetchProducts();
  }, []);
  ...
  const handleProductClick = (product) => {
    navigate(`/product/${product._id}`, {
      state: {
        product: {
          ...product,
          category: "Kurta", // pass the category name for related products
        },
      },
    });
  };
  return (
    <>
      ...
      <div className="bg-[#2c0d0d] pt-24 md:pt-40 px-4 md:px-10 text-white">
        <h1 className="text-3xl font-bold mb-8 text-center">Women's Kurtas</h1>
